...
from tornettools.parse_rusage import parse_resource_usage_logs, extract_resource_usage_plot_data, parse_resource_usage_node_logs, extract_resource_usage_node_plot_data, parse_cpu_node_logs, extract_cpu_node_plot_data
...
def run(args):
    ...
    logging.info("Parsing cpu usage logs for individual nodes.")
    if parse_cpu_node_logs(args):
        logging.info("Extracting cpu usage node plot data.")
        extract_cpu_node_plot_data(args)
    else:
        logging.warning("Parsing cpu usage logs for individual nodes failed, so we cannot extract cpu usage plot data.")
